...
# minus strand: keep row with highest sub_junction_start_coor
a5_dtl_minus <- a5_dtl %>%
  filter(strand == "-") %>%
  group_by(Gene, chr, strand, start, end) %>%
  slice_min(sub_junction_end_coor, n = 1, with_ties = FALSE) %>%
...
# plus strand: keep row with lowest sub_junction_end_coor
a5_dtl_plus <- a5_dtl %>%
  filter(strand == "+") %>%
  group_by(Gene, chr, strand, start, end) %>%
  slice_min(sub_junction_start_coor, n = 1, with_ties = FALSE) %>%
...
a5ss <- a5ss_dtl %>%
  # remove rows where any of the three key columns are NA or Inf
  filter(
    !is.na(deltaPSI), !is.infinite(deltaPSI),
    !is.na(avgwtPSI), !is.infinite(avgwtPSI),
    !is.na(avgUPF1PSI), !is.infinite(avgUPF1PSI)
